...
const post = (body: unknown) =>
  SELF.fetch("https://agents.test/api/backoffice/templates?cf_session=tok", {
...
const patch = (path: string, body: unknown) =>
  SELF.fetch(`https://agents.test/api/backoffice${path}?cf_session=tok`, {
    body: JSON.stringify(body),
    headers: { "Content-Type": "application/json" },
    method: "PATCH",
  });
...
describe("backoffice skill catalog", () => {
  it("returns the full 13-skill code registry with id + label", async () => {
    const res = await SELF.fetch("https://agents.test/api/backoffice/skills?cf_session=tok");
...
describe("backoffice template CRUD", () => {
  it("creates a template (happy path), then lists it", async () => {
    ...
    const createdBody = (await created.json()) as { template: Template };
  ...
  it("updates a template and bumps the version", async () => {
    const created = await post(validBody);
    const { template } = (await created.json()) as { template: Template };
    ...
    const updatedBody = (await updated.json()) as { template: Template };
  ...
  it("retires then restores a template (soft, never hard-deletes)", async () => {
    ...
    expect(((await restored.json()) as { template: Template }).template.status).toBe("active");
  });
