...
// The REST surface as a Hono app factory. The vitest/wrangler entry (index.ts)
// reuses it without `flue()`; the Flue build uses the default export below. Each
// caller gets its own instance so mounting `flue()` on one doesn't affect the other.
...
// plus Flue's agent/workflow/run routes. Per-agent auth (session + CUSTOMER +
// tenant) lives in each agent's `route` export.
