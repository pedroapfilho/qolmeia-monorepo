...
import { buildRoleGuard, requireStaff } from "./require-staff";
...
describe("requireCustomer", () => {
  it("rejects OWNER/STAFF users with 403", async () => {
    const auth = buildAuth(session);
    const prisma = buildPrisma([
      {
        createdAt: new Date("2026-01-01"),
    ...
    const app = buildApp(requireCustomer({ auth, prisma: prisma as never }), (c) =>
      c.json({ ok: true }),
    );
