...
| ----------------- | ------------- | -------------------------------------------------------- | ------------------------ | ------------------- |
| `apps/auth`       | `auth`        | Hono on Node 24 (tsdown → `node dist/index.mjs`)         | **Railway** (+ Postgres) | `auth.qolmeia.com`  |
| `apps/agents`     | `worker-bees` | Cloudflare Worker (DO, Workflows, D1, R2, KV, Vectorize) | **Cloudflare**           | `api.qolmeia.com`   |
...
| **Railway** (or any Node host + managed PG) | `apps/auth` service + Postgres                                                                                | `DATABASE_URL`                             |
...
- `apps/auth` sets `COOKIE_DOMAIN=.qolmeia.com` → Better Auth writes the cookie
...
- `WORKER_PUBLIC_URL=https://api.qolmeia.com`
- `AUTH_SERVICE_URL=https://auth.qolmeia.com`
...
wrangler secret put INTERNAL_SHARED_SECRET  # MUST match apps/auth
...
in the config, map `api.qolmeia.com` to the Worker in the dashboard
(Worker → Settings → Domains & Routes).

## 5. Railway — `apps/auth` + Postgres

Build from the repo root with pnpm (it's a workspace). Build: the monorepo
`pnpm build` (or filtered `--filter=auth`); start: `node dist/index.mjs`
...
| `AGENTS_INTERNAL_URL`    | `https://api.qolmeia.com` (org-create relay target)   |
...
| Var                      | Value                                                                        |
| ------------------------ | ---------------------------------------------------------------------------- |
| `BETTER_AUTH_SECRET`     | same secret as `apps/auth`                                                   |
...
| `NEXT_PUBLIC_AGENTS_URL` | `https://api.qolmeia.com`                                                    |
...
1. **Cloudflare** first — you need `api.qolmeia.com` for `AGENTS_INTERNAL_URL`.
2. **Railway** — auth + Postgres; gives you `auth.qolmeia.com`.
...
until both the Worker and `apps/auth` are redeployed.
