...
> What's shipped on `refactor/architecture-deepening` (HEAD `0cb4dbf`, queued for merge into `main`) and how every piece fits together. This doc supersedes the Phase-4-era version that pre-dated the multi-agent refactor (Phases 5a–5d) and the architecture-deepening pass.

---

## 1. What Qolmeia is (one paragraph)

Qolmeia is an AI workforce platform for Brazilian local businesses. The MVP currently ships **one channel** (a Telegram bot, `@qolmeia_mvp_v0_bot`) and **two seeded agents** working together: a **Controller** that talks to the owner and routes work, and a **Designer** that captures the business "soul," annotates uploaded brand assets, and generates branded images. The Controller delegates to the Designer via a `delegateToSpecialist` skill; future phases add more specialists (Marketing Strategist next). Everything works through a generic agent runtime — the model decides which tool to call based on the message; the host code never imperatively dispatches by intent. The repo is a pnpm + Turborepo monorepo with one app (`apps/api`, Hono on Node) and three packages (`@repo/db`, `@repo/config-vitest`, `@repo/typescript-config`).
...
```
       ...
       └────────────────┬────────────────┬───────────────┬──────────────┘
         ...
         │     ▸ 4 skills, 2 agents, 1 delegation tool                   │
...
AgentAction                                       (the approval queue, awaiting Phase 5f)
  agentInstanceId, skillId
  triggerMessageId?, parentActionId?              for chain traversal
...
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Knowledge Registry redesign (Pedro's pivot)** | A new `KnowledgeDoc` concept: markdown files in R2 with thin DB rows, plus a `searchKnowledge` skill the agent calls to retrieve specific docs by tag/title. Alongside the existing structured `businessProfile` + `BrandAsset` (not replacing them — those stay hot + structured). | `KnowledgeProvider` already abstracts the soul reader; adding a parallel surface for richer docs doesn't touch the existing seam. New skill plugs into the same `defineSkill` factory. |
| **5e — Marketing Strategist**                   | Third template, `canDelegateTo: ["designer"]`. Stub skill `draftMarketingStrategy`. Controller's `canDelegateTo` extends to `["designer", "marketing-strategist"]`. Validates 3-level delegation.                                                                                   | `validateCanDelegateTo` already handles the new DAG. The delegation skill, runtime, and aggregator all support multi-level chains without changes.                                     |
| **5f — AgentAction queue + cost**               | Persist drafts/approvals/executions. `agents/actions.ts` for the lifecycle. Soft-warn at 80%/100% of `AgentInstance.budgetCents`.                                                                                                                                                   | `AgentAction` table is already modeled. `SkillContext.agentInstanceId` is already wired through.                                                                                       |
| **5g — BullMQ dispatcher**                      | Async execution via BullMQ on existing Redis. FlowProducer for delegation parent-child jobs. `workers/agent-runner.ts` as a separate process.                                                                                                                                       | `agents/dispatcher.ts` interface + `agents/main-dispatcher.ts` policy file — swap in one place.                                                                                        |
| **5h — Per-connector webhook routes**           | `POST /connectors/<type>/:connectorInstanceId/webhook`. Migrate `TelegramLink` rows to `ConnectorInstance`. Multi-org capable.                                                                                                                                                      | Schema already has `ConnectorInstance` + binding tables. New routes mount alongside the existing telegram route.                                                                       |
| **v0 web UI**                                   | Owner dashboard, approval queue UI, onboarding wizard. Better Auth for accounts.                                                                                                                                                                                                    | Frontend slice; all data is already in the schema.                                                                                                                                     |
...
- **113 tests** (109 api + 4 db) across 22 files at HEAD `0cb4dbf` (was 92 before the refactor pass; +21 net from the deepening).
...
> "Telegram webhook → Chat SDK adapter → `inbox/pipeline.ts` (5 stages) → `dispatcher.enqueueAndAwait` → `runtime.runAgentInstance(Controller)` → Controller calls `delegateToSpecialist("designer", subtask)` → Designer's `runAgentInstance` runs (extractSoul / labelBrandAsset / generateBrandImage tools) → results bubble up through aggregation → Controller writes final pt-BR reply → handler posts text + any generated images back to Telegram. 113 tests; never silent-fails; single AI key; Prisma+Postgres for data, R2 for binaries, Redis for state."
