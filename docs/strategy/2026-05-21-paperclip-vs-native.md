...
1. **`Company` first-class rename + onboarding** *(3 days)*
...
2. **`Correspondent` role + persistent memory** *(4 days)*
...
3. **Team builder + agent template catalog** *(4 days)*
...
4. **Cross-agent connector sharing** *(2 days)*
...
5. **Slack + Discord adapters** *(3-5 days; depends on Chat SDK fit)*
...
6. **Audio input → transcription** *(2 days)*
...
8. **Polish + integration tests** *(2-3 days)*
...
1. **Paperclip plugin manifest** *(2 days)*
...
2. **Translate skills into Paperclip plugin tools** *(3 days)*
...
3. **Replace `runAgentInstance` with Paperclip session API** *(3 days)*
...
4. **Webhook routes → Paperclip routes** *(2 days)*
...
5. **Customer onboarding + team builder as plugin UI** *(3 days)*
...
6. **Activity log + ticketing translation** *(3 days)*
...
7. **Polish + integration tests** *(2-3 days)*
