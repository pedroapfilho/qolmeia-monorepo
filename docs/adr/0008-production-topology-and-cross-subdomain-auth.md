...
2. **Railway:** deploy `apps/api` + Postgres; env (`DATABASE_URL`, `BETTER_AUTH_SECRET`, `CORS_ORIGINS`, `INTERNAL_SHARED_SECRET` matching the Worker, `RESEND_API_KEY`); `prisma db push`/migrate.
