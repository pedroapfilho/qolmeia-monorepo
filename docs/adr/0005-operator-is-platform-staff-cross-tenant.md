...
The codebase contradicted itself on what an **Operator** is. The dev seed puts the `OWNER` and the `CUSTOMER` in the *same* `company`, and every backoffice route gates on `companyId === session.companyId` — a per-company reading. But the product's differentiator is a human quality layer: Qolmeia professionals who vet AI prompts and results across *all* customer companies. Those can't both be true.
...
The quality layer is a triage problem: the AI Designer's output should reach Qolmeia's *human* designer, copy to a copy reviewer, etc.
